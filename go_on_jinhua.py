...
#从上次留存程序开始
my_list5=open('//media//zhangkun//资料//exchange//AUDUSD//AUDUSDcelue2017115.txt','rb')
...
while True:
    if a%100<60:
        ...
        short_program[a][0] = random.choice([short_program[a-100*(a//100)][0],random.choice(short_program2[0])])
        short_program[a][1] = random.choice([short_program[a-100*(a//100)][1],random.choice(short_program2[1])])
        short_program[a][2] = random.choice([short_program[a-100*(a//100)][2],random.choice(short_program2[2])])
        short_program[a][3] = random.choice([short_program[a-100*(a//100)][3],random.choice(short_program2[3])])
        ...
        #     continue
        # 赚钱则存储，4为终值，5为交易笔数
        short_program[a][4] = strategy.account
        short_program[a][5] = strategy.count
        a += 1
    else:
        # 随机取值
